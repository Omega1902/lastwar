...
def calc_duration(original_seconds: int, buffs: int) -> None:
    ...
    print(
        f"Time without additional buff:  {seconds_to_string(time)} - saved: {seconds_to_string(saved)} ({((saved / time) * 100):.1f}%)"
    )
    time25 = math.ceil(original_seconds / (1 + (buffs + 25) / 100))
    saved25 = time - time25
    print(
        f"Time with additional 25% buff: {seconds_to_string(time25)} - additionally saved: {seconds_to_string(saved25)} ({((saved25 / time) * 100):.1f}%)"
    )
    time50 = math.ceil(original_seconds / (1 + (buffs + 50) / 100))
    saved50 = time - time50
    print(
        f"Time with additional 50% buff: {seconds_to_string(time50)} - additionally saved: {seconds_to_string(saved50)} ({((saved50 / time) * 100):.1f}%)"
    )
